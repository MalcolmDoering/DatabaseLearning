...
encode DB entries and outputs at the level of tokens instead of chars
...
def get_input_output_strings_and_location_vectors(interactions, locationToIndex, spatialStateToIndex, stateTargetToIndex):
    ...
    for turn in interactions:
        ...
        #
        oString = nltk.word_tokenize(oString)
...
def run(gpu, seed, camTemp, attTemp, teacherForcingProb, sessionDir):
    ...
    def prepare_split(startDbIndex, stopDbIndex, splitName):
        ...
        for i in range(startDbIndex, stopDbIndex):
            ...
            #if splitInputSequenceVectors == None:
            #    splitInputSequenceVectors = inputSequenceVectors[i]
            #else:
            #    splitInputSequenceVectors = np.concatenate(splitInputSequenceVectors, inputSequenceVectors[i])
    ...
    def evaluate_split(splitName, splitBatchEndIndices, indexToChar, splitInterestingInstances,
                       ...
                       splitCostAve,
                       splitCostStd,
                       splitInputSequenceVectors=None):
