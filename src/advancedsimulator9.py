...
adapted from advancedSimulator8

don't let the shopkeeper make so many random decisions
"""
...
class CustomerAgent(object):
    ...
    def perform_action(self, prevIntState):
        ...
        currIntState.numGreets = prevIntState.numGreets
        ...
        if prevIntState.turnCount == 0:
            ...
            candidateFeats = {}
            for c in cameras:
                candidateFeats[c] = [x for x in features if x not in currIntState.previousFeaturesOfConversation[c]]
            
            otherCameras = [c for c in cameras if c != prevIntState.outputCustomerLocation]
            ...
            if prevIntState.customerAction == "C_ENTERS":
                ...
                action = self.action_examines_camera
            
            elif prevIntState.customerAction == "C_EXAMINES_CAMERA" and prevIntState.outputShopkeeperAction == "S_NONE":
                
                if len(candidateFeats[prevIntState.outputCustomerLocation]) > 0:
                    action = np.random.choice([self.action_examines_camera, self.action_walks_to_camera, self.action_greets], p=[0.05, 0.05, 0.90])
                
                elif len(candidateFeats[otherCameras[0]]) > 0 or len(candidateFeats[otherCameras[1]]) > 0:
                    action = np.random.choice([self.action_examines_camera, self.action_walks_to_camera], p=[0.10, 0.90])
                
                else:
                    action = self.action_leaves
            
            elif prevIntState.outputShopkeeperAction == "S_GREETS":
                if currIntState.numGreets == 1:
                    action = self.action_looking_for_a_camera
                else:
                    action = self.action_question_about_feature
            
            elif prevIntState.outputShopkeeperAction == "S_INTRODUCES_CAMERA":
                action = np.random.choice([self.action_question_about_feature, self.action_silent_or_backchannel], p=[0.90, 0.10])
            
            elif prevIntState.outputShopkeeperAction == "S_ANSWERS_QUESTION_ABOUT_FEATURE" or prevIntState.outputShopkeeperAction == "S_NOT_SURE" or prevIntState.outputShopkeeperAction == "S_INTRODUCES_FEATURE":
                action = np.random.choice([self.action_question_about_feature, self.action_silent_or_backchannel, self.action_thank_you], p=[0.60, 0.20, 0.20])
            
            elif prevIntState.outputShopkeeperAction == "S_LET_ME_KNOW_IF_YOU_NEED_HELP":
                action = np.random.choice([self.action_walks_to_camera, self.action_leaves], p=[0.40, 0.60])
    ...
    def action_walks_to_camera(self, prevIntState, currIntState):
        ...
        camerasNotTalkedAbout = [c for c in cameras if c not in currIntState.previousCamerasOfConversation]
        
        otherCameras = [c for c in cameras if c != prevIntState.customerLocation]
        random.shuffle(otherCameras)
        
        intersection = [c for c in camerasNotTalkedAbout if c in otherCameras]
        ...
        if len(intersection) != 0:
            cam = np.random.choice(intersection)
        
        
        # if the other cameras have both already been talked about, then go to one that still has undiscussed features
        #elif len(candidateFeats[otherCameras[0]]) > 0:
        #    cam = otherCameras[0]
        #elif len(candidateFeats[otherCameras[1]]) > 0:
        #    cam = otherCameras[1]
        ...
        # if there are no undiscussed features for the other cameras, then just leave instead
        if cam == None:
            currIntState = self.action_leaves(prevIntState, currIntState)        
    ...
    def action_question_about_feature(self, prevIntState, currIntState):
        ...
        # just say thank you instead of asking about another feature
        else:
            currIntState.customerAction = "C_THANK_YOU"
            currIntState.customerTopic = "NONE"
...
class ShopkeeperAgent(object):
    ...
    def perform_action(self, prevIntState, currIntState):
        ...
        if prevIntState.turnCount == 0:
            ...
            if currIntState.customerAction == "C_ENTERS":
                action = self.action_none
                ...
                if currIntState.numGreets == 0:
                    action = np.random.choice([self.action_none, self.action_greets], p=[0.90, 0.10])
                else:
                    action = self.action_none
            
            elif currIntState.customerAction == "C_GREETS":
    ...
    def action_introduces_feature(self, currIntState):
        ...
        # if all features have already been introduced, return to counter
        else:
            currIntState = self.action_let_me_know_if_you_need_help(currIntState)
...
def run_simulator(sessionDir, databaseFilename, shopkeeperUtteranceFilename, customerUtteranceFilename, randomSeed):
    ...
    print("ave interaction len is", round(np.mean(interactionLens), 2), "s.d.", round(np.std(interactionLens), 2))
