...
class DMN_smooth:
    ...
    def __init__(self,
        ...
        inputs, questions, answers, input_masks = data
        
        self.train_input = inputs
        self.train_q = questions
        self.train_answer = answers
        self.train_input_mask = input_masks
        
        self.test_input = inputs
        self.test_q = questions
        self.test_answer = answers
        self.test_input_mask = input_masks
        ...
        self.W_inp_res_in = nn_utils.normal_param(std=0.1, shape=(self.dim, self.char_vector_size))
        ...
        self.W_inp_upd_in = nn_utils.normal_param(std=0.1, shape=(self.dim, self.char_vector_size))
        ...
        self.W_inp_hid_in = nn_utils.normal_param(std=0.1, shape=(self.dim, self.char_vector_size))
    ...
    def _process_input(self, data_raw):
        ...
        for x in data_raw:
            ...
            inp_vector = [dmnutils.process_word(word = w, 
                                        word2vec = self.word2vec, 
                                        vocab = self.vocab, 
                                        ivocab = self.ivocab, 
                                        word_vector_size = self.char_vector_size, 
                                        to_return = "word2vec") for w in inp]
                                        
            q_vector = [dmnutils.process_word(word = w, 
                                        word2vec = self.word2vec, 
                                        vocab = self.vocab, 
                                        ivocab = self.ivocab, 
                                        word_vector_size = self.char_vector_size, 
            ...
            answers.append(dmnutils.process_word(word = x["A"], # TODO: add .lower() here 
                                            word2vec = self.word2vec, 
                                            vocab = self.vocab, 
                                            ivocab = self.ivocab, 
                                            word_vector_size = self.char_vector_size, 
    ...
    def step(self, batch_index, mode):
        ...
        param_norm = np.max([dmnutils.get_norm(x.get_value()) for x in self.params])
